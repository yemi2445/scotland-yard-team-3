...
export const InteractiveMap: React.FC<InteractiveMapProps> = ({ players, currentRound, isLecturer = false, gameOver = false, gamePin, currentPlayerId, currentTurn, selectedTransport = null, onMove, mapId = DEFAULT_MAP_ID }) => {
    ...
    const constrainPos = useCallback(
        (x: number, y: number, s: number) => {
            ...
            if (scaledW < containerDims.width) {
                ...
                const minX = containerDims.width - scaledW;
            ...
            if (scaledH < containerDims.height) {
                ...
                const minY = containerDims.height - scaledH;
    ...
    const handleMouseMove = (e: React.MouseEvent) => {
        ...
        setPos(constrainPos(nextX, nextY, scale));
    };
    ...
    const handleWheel = (e: React.WheelEvent) => {
        ...
        const newScale = Math.max(getMinScale(), Math.min(scale + delta, 5));

        const rect = containerRef.current?.getBoundingClientRect();
        if (rect) {
            const mouseX = e.clientX - rect.left;
            const mouseY = e.clientY - rect.top;
            const imageMouseX = (mouseX - pos.x) / scale;
            const imageMouseY = (mouseY - pos.y) / scale;
            const nextX = mouseX - imageMouseX * newScale;
            const nextY = mouseY - imageMouseY * newScale;
    ...
    return (
        <div ref={containerRef} className={styles.mapContainer} onMouseDown={handleMouseDown} onMouseMove={handleMouseMove} onMouseUp={handleMouseUp} onMouseLeave={handleMouseUp} onWheel={handleWheel}>
            ...
            >
                <img ref={imgRef} src={MAP_IMAGES[mapDefinition.id].src} alt={`${mapDefinition.name} Map`} className={styles.mapImage} onLoad={handleImageLoad} draggable={false} />
                {imgDims.width > 0 && (
                    <>
                        <svg
                            style={{
                        ...
                        >
                            ...
                            {MAP_EDGES.map((edge) => {
                                ...
                                const isMyRoute = isMyTurn && !!selectedTransport && edge.type === selectedTransport && edge.from === currentPlayerObj?.position && filteredValidMoves.has(edge.to);
                                const isConnectedToHover = !isMyTurn && hoveredNodeId !== null && (edge.from === hoveredNodeId || edge.to === hoveredNodeId);
                                const highlight = isMyRoute || isConnectedToHover;
                        ...
                        {MAP_NODES.map((node) => (
                            <PositionNode
                                ...
                                x={node.x}
                                y={node.y}
                                highlight={getNodeHighlight(node.id)}
                                onClick={(e) => handleNodeClick(node.id, e)}
                                onMouseEnter={() => !isMyTurn && setHoveredNodeId(node.id)}
                        ...
                        {Object.entries(playersByPosition).map(([posId, group]) => {
                            ...
                            const dim = (isMyTurn && !!selectedTransport) || (!isMyTurn && hoveredNodeId !== null);

                            return <PlayerIcon key={posId} players={group} x={(node.x / 100) * imgDims.width} y={(node.y / 100) * imgDims.height} dim={dim} />;
                        })}
                    </>
                )}
            </div>
